...
void AClimbingCharacter::OnStateExit(EClimbingState OldState)
{
	...
	if (NewState == EClimbingState::None)
	{
		...
		// Restore camera probe radius (default is typically 12.0f)
		if (CameraBoom && IsLocallyControlled())
		{
			CameraBoom->ProbeSize = 12.0f;  // UE5 default
		}
...
void AClimbingCharacter::Server_AttemptGrab_Implementation(FClimbingDetectionResultNet ClientResult)
{
	...
	if (ClimbingMovement)
	{
		...
		if (ServerDetection.HitComponent.IsValid())
		{
			ClimbingMovement->SetAnchor(ServerDetection.HitComponent.Get(), ServerDetection.LedgePosition);
		}
	}
